...
3. Install ubuntu in termux
   ...
   ```curl -LSs "https://raw.githubusercontent.com/GhostMaster69-dev/crave-in-termux/refs/heads/master/ubuntu-in-termux.sh" | bash```

4. Start ubuntu in termux

   ```./startubuntu.sh```

5. Run following commands:

   ```apt update && DEBIAN_FRONTEND=noninteractive apt install curl -y && curl -LSs "https://raw.githubusercontent.com/GhostMaster69-dev/crave-in-termux/refs/heads/master/crave-in-termux.sh" | bash```

6. Next time the command ```./startubuntu.sh``` give you the direct access to your devspace in termux.
